...
IMAGE_FILE=""
declare -a IMAGES  # Array to hold loaded images
...
EXAMPLES:
    ...
    $0 --image-file custom.txt --use-existing  # Use custom file with existing SBOMs
...
load_images() {
    ...
    if [ -f "$file_to_use" ] && [ -s "$file_to_use" ]; then
        ...
        if [ ${#IMAGES[@]} -eq 0 ]; then
            ...
            echo -e "${GREEN}✓ Loaded ${#IMAGES[@]} images from $file_to_use:${NC}"
...
```bash
./syft-compare.sh --image-file my-images.txt
```
...
print_results_table() {
    ...
    for image in "${IMAGES[@]}"; do
        ...
        if [[ -f "$OUTPUT_DIR/trivy_sbom_${safe_name}_counts.txt" ]]; then
            ...
            printf "%-35s ${CYAN}%-8s${NC} %-6s %-6s %-6s %-6s %-6s %-6s %-12s %-12s %-12s %-12s %-12s\n" \
        ...
        if [[ -f "$OUTPUT_DIR/grype_sbom_${safe_name}_counts.txt" ]]; then
            ...
            printf "%-35s ${PURPLE}%-8s${NC} %-6s %-6s %-6s %-6s %-6s %-6s %-12s %-12s %-12s %-12s %-12s\n" \
